...
## HUD overlay for Act 1.3: voltage slider, three gear selectors, three RPM
## gauges, diagnostic panel, win panel.
...
func bind(state: Node) -> void:
	...
	state.connect("gear_train_changed", _on_gears)
...
func _ready() -> void:
	...
	_populate_gear_select(get_node(idler_gear_select_path))
...
func _on_idler_gear_picked(idx: int) -> void:
	_send_gears()
...
func _send_gears() -> void:
	...
	var isel: OptionButton = get_node(idler_gear_select_path) as OptionButton
	var nsel: OptionButton = get_node(driven_gear_select_path) as OptionButton
	if dsel == null or isel == null or nsel == null:
		return
	var d: int = int(dsel.get_selected_id())
	var i: int = int(isel.get_selected_id())
	var dn: int = int(nsel.get_selected_id())
	var bc: Node = get_node_or_null("/root/Bridge")
	if bc != null:
		bc.send_action("set_gears", {"driver": d, "idler": i, "driven": dn})
...
func _on_joint(joint_id: int, rpm: float, _angle: float) -> void:
	if joint_id == 0:
		...
		_set_bar(get_node(idler_rpm_label_path), get_node(idler_bar_path), rpm)
	elif joint_id == 2:
...
func _on_gears(d_teeth: int, i_teeth: int, dn_teeth: int) -> void:
	if d_teeth > 0:
		var dsel: OptionButton = get_node(driver_gear_select_path) as OptionButton
	...
	if i_teeth > 0:
		var isel: OptionButton = get_node(idler_gear_select_path) as OptionButton
		if isel != null and isel.get_selected_id() != i_teeth:
			var idx: int = TEETH_OPTIONS.find(i_teeth)
			if idx >= 0:
				_sending = true
				isel.selected = idx
				_sending = false
	if dn_teeth > 0:
		...
		if nsel != null and nsel.get_selected_id() != dn_teeth:
			...
			if idx2 >= 0:
				...
				nsel.selected = idx2
				_sending = false
	# Ratio: driver teeth : driven teeth (idler cancels out, so not in ratio).

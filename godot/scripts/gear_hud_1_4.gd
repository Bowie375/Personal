...
## HUD overlay for Act 1.4 (loaded geartrain): voltage slider, load slider,
## two gear selectors, two RPM gauges, output-torque readout, stall banner,
...
var _max_load: float = 4.0  # slider range N·m; target load (1.5) sits mid-range
...
func _populate_gear_select(select: Node) -> void:
	...
	ob.selected = 3  # index of 30

func _on_driver_gear_picked(_idx: int) -> void:
	_send_gears()

func _on_driven_gear_picked(_idx: int) -> void:
	_send_gears()
...
func _on_joint(joint_id: int, rpm: float, _angle: float) -> void:
	...
	# Torque readouts depend on live motor state (back-EMF changes tau), so
	# refresh them every joint update.
	_update_output_torque()
	...
	if dn_teeth > 0:
		...
		if nsel != null and nsel.get_selected_id() != dn_teeth:
			...
			if idx2 >= 0:
				...
				nsel.selected = idx2
				_sending = false
	# Ratio + reduction readout.
	if d_teeth > 0 and dn_teeth > 0:
		var ratio_lbl: Label = get_node(ratio_value_label_path) as Label
		if ratio_lbl != null:
			var g: int = _gcd(d_teeth, dn_teeth)
			ratio_lbl.text = "N%d : N%d  (%.2f:1 reduction)" % [
				d_teeth / g, dn_teeth / g, float(dn_teeth) / float(d_teeth)
			]
		_update_output_torque()

func _update_output_torque() -> void:
	# The load slider sets the resisting torque on the DRIVEN (output) shaft, so
	# output_torque == load_torque (when running) and the motor fights the
	# REFLECTED load (load / reduction). Both come straight from the backend
	# extras — we don't recompute here, so the numbers match the sim exactly.
	if _state == null:
		return
